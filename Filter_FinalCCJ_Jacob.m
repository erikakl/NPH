...
folder_subjects = strcat(pwd,'/Healthy_Subjects/Jacob');
...
if p == 1
    mask_image(147:152,197,:) = 0;
end
if p == 2
    mask_image(150:151,197,:) = 0;
    mask_image(162,194,:) = 0;
end
...
%4095=6cm/s (V_enc), 4095/2=0cm/s, 0=-6cm/s(-V_enc)
for k=1:svv(3)
    tmp = double(velocity_image(:,:,k));
    tmp = ((2*V_enc)/4095)*tmp-V_enc;
    vk(k) = sum(tmp(idx));
...
for k = 2:svv(3);
    for i = 1:svv(2)
        for j = 1:svv(2)
            if abs(TMP(i,j,k) - TMP(i,j,k-1)) > 1.5*V_enc && abs(TMP(i,j,k)) ~=0
...
% fid = fopen(['Jacob',subject_folders(p).name,'_VolumetricCSFFlow'],'w');
% for i = 1:svv(3)
...
fid2 = fopen(['Jacob',subject_folders(p).name,'_Peakvel_Av'],'w');
for i = 1:svv(3)
    fprintf(fid2, '%f %f %f\n',time(i),storepeak(i,1),storepeak(i,2));
end
fclose(fid2);

% fid3 = fopen(['Jacob',subject_folders(p).name,'_MeanVelCSF'],'w');
% for i = 1:svv(3)
%     fprintf(fid3, '%f %f\n',time(i),vk(i));
% end
% fclose(fid3);
% 
% fid4 = fopen(['Jacob',subject_folders(p).name,'_PressGradCSF'],'w');
% for i = 1:svv(3)
%     fprintf(fid4, '%f %f\n',time(i),dp_mean(i));
% end
% fclose(fid4);
