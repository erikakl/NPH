...
th_max = 2;%3.5;
th_al = 5.2; %5.5
%NB! Special treatment for Subject_22 due to psyko aliasing. First frame
%need to be filtered like: for i=126:svv(3) and j=155:svv(3)
...
for k = 2:svv(3);%start+1:svv(3);
    for i = 1:svv(2)
        for j = 1:svv(2)
            if abs(TMP(i,j,k) - TMP(i,j,k-1)) > 0.9*V_enc && abs(TMP(i,j,k)) ~=0
