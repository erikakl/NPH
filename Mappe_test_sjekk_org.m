...
folder_subjects = strcat(pwd,'/test');
...
xylim = ([130 180 130 180]);
Nb = 8;
velocity_image = zeros(velocity_volume.dim(1),velocity_volume.dim(2),velocity_volume.dim(3)); 
mask_image = flipdim(spm_slice_vol(mask_volume,spm_matrix([0 0 Nb]),mask_volume.dim(1:2),1),2);
figure;imagesc(fliplr(mask_image));axis(xylim);caxis([-1 1]*12);colorbar
title('mask image')
...
for i = 1:svv(1)
    for j = 2:svv(2)
        if tmpMask(i,j) < 2.0485 && tmpMask(i,j) > 2.0475
           tmpMask(i,j) = 0;
        else
            tmpMask(i,j) = 1;
        end
...
mask_image = fliplr(tmpMask);
mask_image(1:288,288) = 0;
...
for n = 1:velocity_volume.dim(3)
    ...
    %figure
    %imagesc(velocity_image(:,:,n))
    %title(num2str(n));axis(xylim);caxis([-1 1]*12)
end 
figure
imagesc(velocity_image(:,:,Nb))
title(num2str(Nb));axis(xylim);caxis([-1 1]*12);colorbar
...
fid = fopen([subject_folders(p).name,'_VolumetricCSFFlow_NoFilt'],'w');
for i = 1:svv(3)
    fprintf(fid, '%f %f %f %f\n',time(i),Q(i,1),Q(i,2),Q(i,3));
end
fclose(fid);

fid2 = fopen([subject_folders(p).name,'_VolumetricCSFFlowInt_NoFilt'],'w');
for i = 1:svv(3)
    fprintf(fid2, '%f %f\n',time(i),Qint(i));
end
fclose(fid2);

fid3 = fopen([subject_folders(p).name,'_MeanVelCSF_NoFilt'],'w');
for i = 1:svv(3)
    fprintf(fid3, '%f %f\n',time(i),vk(i));
end
fclose(fid3);

fid4 = fopen([subject_folders(p).name,'_PressGradCSF_NoFilt'],'w');
for i = 1:svv(3)
    fprintf(fid4, '%f %f\n',time(i),dp_mean(i));
end
fclose(fid4);
