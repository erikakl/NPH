...
%NB!!!!!!!!!!!
% p=5 suffers from heavy aliasing, I have removed 3 pixles see line 75
%folder_subjects = strcat(pwd,'/Subjects_nifti');
%The masked region has the value 2.047500133514404...
%edge have to be fixed as well

folder_subjects = strcat(pwd,'/Healthy_Subjects/Kent');
...
for i = 1:svv(1)
    for j = 2:svv(2)
        if tmpMask(i,j) < 2.04751 && tmpMask(i,j) > 2.0475
           tmpMask(i,j) = 0;
        else tmpMask(i,j) = 1;
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%NOTE!!!! p=1,2,3,5, special treatment due to heavy aliasing
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if p == 1
    mask_image(128,170) = 0;
    mask_image(132,184) = 0;
    mask_image(160,176) = 0;
end
if p == 2
    mask_image(130,181) = 0;
end
if p == 3
    mask_image(149,186) = 0;
    mask_image(151,185)=0;
end
if p == 5
    mask_image(134,175) = 0;
    mask_image(135,176)=0;
    mask_image(139,179)=0;
end
...
% fid = fopen(['Kent',subject_folders(p).name,'_VolumetricCSFFlow'],'w');
% for i = 1:svv(3)
%     fprintf(fid, '%f %f %f %f\n',time(i),Q(i,1),Q(i,2),Q(i,3));
% end
% fclose(fid);

fid2 = fopen(['Kent',subject_folders(p).name,'_Peakvel_Av'],'w');
for i = 1:svv(3)
    fprintf(fid2, '%f %f %f\n',time(i),storepeak(i,1),storepeak(i,2));
end
fclose(fid2);
% fid3 = fopen(['Kent',subject_folders(p).name,'_MeanVelCSF'],'w');
% for i = 1:svv(3)
%     fprintf(fid3, '%f %f\n',time(i),vk(i));
% end
% fclose(fid3);
% 
% fid4 = fopen(['Kent',subject_folders(p).name,'_PressGradCSF'],'w');
% for i = 1:svv(3)
%     fprintf(fid4, '%f %f\n',time(i),dp_mean(i));
% end
% fclose(fid4);
